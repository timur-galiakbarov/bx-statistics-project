...
## 2026-08-24 - Первый проход по дизайну под старый кабинет

Запрос пользователя:

- Не добавлять audit log сейчас.
- Подогнать дизайн новой React-версии под предыдущий интерфейс Socstat.

Изучено в старом frontend:

- Основной шаблон: `front/src/template/app/template.html`.
- Базовая тема: `front/src/template/app/material-dashboard.css`.
- Кастомные стили: `front/src/template/app/app-styles.css`.
- Старый кабинет использовал Material Dashboard:
  - фиксированный левый sidebar шириной около `260px`;
  - фон sidebar из `front/images/sidebar-1.jpg`;
  - светлая рабочая область `#eeeeee`;
  - белые карточки `block-flat/card` с лёгкой material-тенью;
  - зелёный активный пункт меню `#4caf50`;
  - простые rectangular-кнопки с небольшим радиусом;
  - topbar без тяжёлой заливки.

Решение:

- Базовая тема React-приложения переведена ближе к старому Material Dashboard.
- Sidebar теперь использует старый фон `sidebar-1.jpg` со светлым overlay.
- Рабочая область переведена на серый фон `#eeeeee`.
- Панели получили material-like shadow и меньший border-radius.
- Активный пункт меню и основные кнопки переведены на `#4caf50`.
- Topbar стал легче и ближе к старому navbar.
- Формы и select получили стиль с нижней границей вместо плотных bordered inputs.
- Таблицы стали ближе к старым: тонкие разделители, прозрачный header, uppercase labels.
- Статусы оплат сделаны цветными rectangular labels.
- Карточки постов, метрики и тарифы подогнаны под общий material-like стиль.
...
- `npm run build`
- Проверено, что Vite корректно собрал старую картинку sidebar в `dist/assets`.
- Проверено, что frontend dev server уже запущен на `5173`, API на `4000`.

Следующие шаги:

- В браузере пройти `/dashboard`, `/analytics`, `/posts`, `/account`, `/admin` и отметить, какие блоки всё ещё выглядят слишком новыми.
- Вторым дизайн-проходом точечно привести страницы анализа и карточки постов к старым шаблонам `block-flat`, `fd-tile`, `post-default`.

## 2026-08-24 - Исправление заголовка страницы при навигации

Запрос пользователя:

- Заголовок страницы не меняется при переходе со страницы на страницу.

Причина:

- `App` считал заголовок через `window.location.pathname` внутри `useMemo`.
- React Router меняет location без реактивной подписки на `window.location`, поэтому компонент не пересчитывал заголовок при клиентской навигации.

Решение:

- В `App` подключён `useLocation()` из `react-router-dom`.
- Заголовок теперь считается от `location.pathname`.
- Проверки `/login` и `/auth/vk/implicit-callback` тоже переведены на `location.pathname`.
...
## 2026-08-24 - Фиксация фоновой картинки сайдбара

Запрос пользователя:

- Фоновая картинка в сайдбаре двигается в зависимости от высоты экрана, нужно сделать её статичной.

Решение:

- Для `.sidebar` фоновая картинка отделена от shorthand-значения `center / cover`.
- Позиция фона зафиксирована как `center top`, размер оставлен `cover`.
- Логин-экран не менялся.
...
## 2026-08-24 - UX-концепция для SMM-специалиста

Запрос пользователя:

- Перед переработкой интерфейса нужно подумать, что нужно SMM-специалисту.
- Основной фокус сервиса: не просто метрики, а рабочий инструмент для анализа сообществ и публикаций.

Решение:

- Создан отдельный документ `docs/ux-redesign.md`.
- Зафиксирован основной пользователь: SMM-специалист.
- Зафиксирован главный принцип UX: данные должны вести к выводам и действиям.
- Центральной страницей MVP определён раздел “Анализ сообществ”.
- Описана структура страницы: верхняя панель, KPI, “Что важно”, график, эффективность контента, карточки постов, форматы, время публикаций, таблица.
- Зафиксирован MVP-порядок реализации.
...
- Документ создан без изменения runtime-кода.

## 2026-08-24 - Первый UX-проход страницы “Анализ сообществ”

Запрос пользователя:

- Приступить к коду по UX-переработке страницы “Анализ сообществ”.

Решение:

- Страница `AnalyticsPage` перестроена из вкладочного отчёта в линейный рабочий экран SMM-специалиста.
- Верх страницы теперь содержит выбор сообщества, период и кнопку обновления.
- Добавлена KPI-сводка: подписчики, прирост, охват, посещения, активность, ER, публикации.
- Добавлен блок “Что важно” с простыми rule-based выводами по доступным данным.
- Добавлен один главный график с переключателями: охват, активность, ER.
- Добавлен блок “Контентная эффективность” с сегментами: топ постов, слабые, выше среднего, ниже среднего.
- Ниже оставлен детальный список всех публикаций с сортировкой и компактной таблицей метрик.
- Существующий `PostCard` переиспользован для карточек публикаций.
...
- `npm run build`
- Визуальная browser-проверка не выполнена: Browser runtime вернул пустой список доступных браузеров.

## 2026-08-24 - Быстрый выбор сообщества в анализе

Запрос пользователя:

- Стартовый экран `Анализ сообществ` выглядит пустым: пользователь не видит, какое сообщество выбрать.
- Показывать последние анализируемые и избранные/сохранённые сообщества, затем дать возможность искать другое.

Решение:

- На первом экране добавлен блок выбора сообщества с приоритетом: недавно анализированные, избранные, остальные сохранённые.
- История последних анализов хранится локально в браузере, содержит до пяти сообществ и обновляется после каждого запуска анализа.
- После выбора вместо большого стартового списка остаётся компактная панель выбранного сообщества с действием `Сменить сообщество`.
- Поиск по названию, screen name или ссылке VK остаётся доступен для нового сообщества.
...
- Визуально проверить стартовый экран с пустой историей, с сохранёнными группами и после повторного входа в раздел.

## 2026-08-24 - Закрытие результатов поиска после выбора сообщества

Запрос пользователя:

- После клика по найденному в поиске сообществу список результатов остаётся на экране и создаёт неочевидное состояние.

Решение:

- При запуске анализа выбранного сообщества результаты поиска очищаются вместе с закрытием панели выбора.
- После клика пользователь видит только компактную панель выбранного сообщества и состояние загрузки анализа.
...
## 2026-08-24 - UX-переработка страницы «Анализ сообществ»

Запрос пользователя:

- Переработать рабочий сценарий страницы с позиции SMM-специалиста: за 30–60 секунд показать динамику сообщества, причину результата, сильный и слабый контент и следующее действие.

Решение:

- API аналитики расширен сравнением с предыдущим аналогичным периодом для прироста, посещений, охвата сообщества и метрик публикаций.
- Добавлены периоды: 7, 14 и 30 дней, текущий месяц, предыдущий месяц и произвольный диапазон.
- Охват сообщества остаётся отдельной метрикой `stats.get`; в анализе публикаций используются корректные названия «просмотры» и «средние просмотры поста».
- KPI показывают текущее значение, изменение, состояние и tooltip с формулой/источником. Отсутствующие права `stats` показываются как недоступные данные, а не как нули.
- Если VK вернул только первые 100 публикаций и предыдущий период нельзя проверить целиком, сравнение wall-метрик явно помечается недоступным.
- Выводы «Что важно» строятся по схеме факт → интерпретация → действие, учитывают изменения периода, ER, просмотры, частоту публикаций, рекламу и эффективность форматов на публикацию.
- В контентной эффективности добавлены критерии ER, реакции, просмотры, репосты и комментарии, а также фильтр органических и рекламных публикаций.
- Карточка публикации дополнена типом контента, органическим/рекламным статусом, бейджем результата и объяснением попадания в сегмент.
- Верхняя панель получила действия обновления, открытия VK-сообщества и копирования готового текстового вывода.
- Добавлены skeleton-состояние загрузки и адаптивные правила для новых контролов.
...
- `npm run build`
- `git diff --check`
- Неавторизованный запрос к `/api/analytics/community/:groupId` вернул ожидаемый `401`.
- Визуальная проверка через Browser runtime не выполнена: в окружении нет доступного браузера.

Ограничения:

- VK `wall.get` в текущем сервисе загружает до 100 последних публикаций. Если этого недостаточно, backend не показывает недостоверное сравнение с предыдущим периодом.
- Охват и посещения зависят от права `stats` у VK token.

Следующие шаги:

- Проверить экран в браузере на сообществе с публикациями в обоих периодах и на узком мобильном экране.
- При необходимости добавить пагинацию wall.get на backend для полного сравнения длинных периодов у активно публикующих сообществ.

## 2026-08-25 - Достоверность периодов и детальный режим аналитики

Запрос пользователя:

- После UX-аудита сделать сравнение периодов достоверным, добавить тесты и не показывать уверенные выводы для неполной или недостаточной выборки.

Результат:

- Расчёт периодов, предыдущих периодов, полноты стены и дневных рядов вынесен в тестируемый `analyticsUtils`.
- Исправлены календарные границы текущего и предыдущего месяцев, в том числе для 29, 30 и 31 числа; произвольный период ограничен 93 днями и не может включать будущие даты.
- `wall.get` загружается страницами до начала предыдущего сравниваемого периода, но не более 1 000 публикаций. API отдельно помечает полноту текущей и предыдущей выборки.
- Временные ряды включают все дни периода: без публикаций реакции равны `0`, а ER и средние просмотры имеют `null`.
- В графике текущий и предыдущий периоды сопоставляются по номеру дня в периоде, а не по позиции отфильтрованного массива.
- В блоке эффективности нулевое среднее больше не создаёт ложных топов и процентов; выводы о формате используют только органические посты, минимум три публикации и медианный ER.
- Возвращён детальный режим всех публикаций: поиск, фильтр органических/рекламных, сортировка, счётчик и сброс фильтров.
- На мобильном экране детальная таблица становится подписанными карточками без горизонтального чтения чисел.

Изменённые файлы:

- `apps/api/package.json`
- `apps/api/src/services/analyticsUtils.ts`
- `apps/api/src/services/analyticsUtils.test.ts`
- `apps/api/src/services/analyticsService.ts`
- `apps/frontend/src/api/types.ts`
- `apps/frontend/src/pages/AnalyticsPage.tsx`
- `apps/frontend/src/components/AnalyticsChart.tsx`
...
- `npm test`: 5 тестов пройдены, TypeScript frontend и backend прошёл.
- `npm run build`: пройден.
- `git diff --check`: пройден.

Ограничения:

- После 1 000 публикаций wall-история считается неполной и сравнительные wall-метрики отключаются.
- Для охвата, посещений и прироста всё ещё требуется scope `stats` у VK token.

## 2026-08-25 - Главная как диспетчерская SMM-специалиста

Запрос пользователя:

- Переработать «Главную» в обзор портфеля сообществ и точку входа в рабочие сценарии, не дублирующую подробный анализ.

Результат:

- Старые постоянные блоки подписок и бесплатных групп убраны с первого экрана.
- Добавлены: сигналы «Требует внимания», четыре KPI портфеля, быстрые действия и карточки портфеля с фильтрацией/сортировкой.
- Ошибка отдельного сообщества, отсутствие статистики VK и отрицательный прирост показаны как разные сигналы, без подмены данных нулями.
- Добавлен onboarding для пустого портфеля.
- Поиск и добавление сообщества перенесены в компактный блок «Управление сообществами».
- На мобильном экране карточки портфеля и сигналы перестраиваются в одноколоночный вид.
...
- Текущий dashboard API пока не отдаёт сопоставимые прошлые ER и средние просмотры по каждой группе, поэтому Главная не генерирует по ним ложные сигналы.

## 2026-08-25 - URL выбранного сообщества в анализе

Запрос пользователя:

- При запуске анализа сообщества всегда добавлять его id в URL.

Результат:

- После выбора или поиска сообщества URL меняется на `/analytics?groupId=:id`.
- При изменении периода идентификатор остаётся в URL, поэтому страницу можно обновить или передать ссылку на конкретное сообщество.
...
## 2026-08-25 - Независимый сайдбар и фоновая картинка

Запрос пользователя:

- Сайдбар не должен растягивать фоновую картинку по высоте контента и зависеть от прокрутки страницы.

Результат:

- На desktop сайдбар закреплён относительно viewport, имеет высоту экрана и собственную прокрутку при необходимости.
- Основная область всегда остаётся во второй колонке grid.
- Фоновое изображение масштабируется по ширине сайдбара, а не по его высоте, и закреплено у верхнего края.
- На мобильной ширине сайдбар остаётся обычным блоком над контентом.
...
## 2026-08-25 - Позиция фоновой картинки сайдбара

Запрос пользователя:

- Прижать фоновую картинку сайдбара к низу экрана.

Результат:

- Overlay остаётся на всю высоту сайдбара, а изображение фона выровнено по центру нижнего края viewport.
...
## 2026-08-25 - Ограниченное масштабирование фона сайдбара

Запрос пользователя:

- Растягивать фоновую картинку по высоте экрана, но ограничить её размер на высоких экранах.

Результат:

- Фоновое изображение растягивается до высоты viewport, но не более чем до `900px`, сохраняя пропорции и нижнее выравнивание.
...
## 2026-08-25 - Увеличение лимита высоты фона сайдбара

Запрос пользователя:

- Увеличить ограничение высоты фоновой картинки с `900px` до `1400px`.

Результат:

- Максимальная высота изображения фона сайдбара увеличена до `1400px`.
...
## 2026-08-25 - Возврат footer кабинета

Запрос пользователя:

- Вернуть footer с текстом «Нашли ошибку? Сообщите нам» и ссылкой на VK.

Результат:

- Footer добавлен в общий layout кабинета и отображается на всех его страницах.
- Ссылка ведёт на `https://vk.com/socstat_ru`.
...
## 2026-08-25 - Удаление раздела «Каналы» из кабинета

Запрос пользователя:

- Убрать бесполезную страницу «Каналы».

Результат:

- Пункт «Каналы» удалён из навигации.
- Route `/channels` удалён, поэтому раздел больше не доступен из React-кабинета.
- Исходный компонент сохранён как неиспользуемый код, чтобы не выполнять необратимое удаление без необходимости.
...
## 2026-08-25 - Импорт Bitrix-пользователей и VK-токенов в MongoDB

Запрос пользователя:

- Добавить `legacy.bitrixId` отдельным свойством пользователя.
- Перенести VK access token, не перенося Bitrix-хеши паролей.
- Для дублирующихся VK ID оставить только наиболее новую запись.

Результат:

- В `users` добавлены поля `legacy.bitrixId`, `email`, `isActive`, `lastLoginAt` и `lastActivityAt`; `vkId` стал необязательным и уникальным при наличии.
- Создан идемпотентный импорт `import:legacy-users`, переносящий пользователей и токены в `vktokens`.
- Импортировано 58 609 Bitrix-пользователей и 56 192 VK-токена; две устаревшие записи с дублирующимися VK ID исключены.
- После импорта в базе 58 610 пользователей, из них 58 609 имеют `legacy.bitrixId`; дублирующихся `vkId` нет.
- У импортированных пользователей и токенов установлены служебные даты `createdAt`/`updatedAt`, нужные существующим репозиториям API.
...
- `apps/api/src/repositories/accountRepository.ts`
- `apps/api/src/routes/payments.ts`
- `apps/api/src/scripts/importLegacyBitrixUsers.ts`
- `apps/api/package.json`
- `docs/migration-log.md`

Проверка:

- `npm run build --workspace @socstat/api`
- `npm run test --workspace @socstat/api`
- Агрегированная сверка MongoDB: количество пользователей и токенов, уникальность `vkId`, наличие актуальных записей дубликатов.

## 2026-08-25 - Последние вошедшие пользователи в админке

Запрос пользователя:

- Добавить в админку блок последних активных пользователей.
- Выводить до 300 пользователей, которые действительно входили в систему.

Результат:

- Добавлен защищённый админский маршрут `GET /api/account/admin/users/recent`.
- Выборка ограничена на сервере 300 записями и сортируется по `lastLoginAt` по убыванию.
- В админке отображаются время последнего входа, имя, email, VK ID, Bitrix ID и дата доступа; есть кнопка обновления.
- При новом VK-входе поле `lastLoginAt` обновляется.
...
## 2026-08-25 - Статус учётной записи в списке активных пользователей

Запрос пользователя:

- Добавить столбец «активный / не активный» в список последних вошедших пользователей.

Результат:

- API возвращает поле `isActive` для каждого пользователя.
- В админке добавлен цветной столбец «Статус» со значениями «Активный» и «Не активный».
...
## 2026-08-25 - Удаление email из списка последних входов

Запрос пользователя:

- Убрать email из списка последних вошедших пользователей в админке.

Результат:

- Email удалён из интерфейса и больше не возвращается маршрутом списка пользователей.
...
## 2026-08-25 - Статус доступа вместо статуса Bitrix-учётной записи

Запрос пользователя:

- Показывать фактический активный доступ, а не исторический `isActive` из Bitrix.

Результат:

- Столбец переименован в «Доступ».
- Его значение рассчитывается по условию `activeTo > now`: «Активен» при действующей подписке, иначе «Не активен».
...
## 2026-08-25 - Импорт исторических платежей Bitrix

Запрос пользователя:

- Перенести все платежи из нормализованной выгрузки Bitrix.

Результат:

- В модель платежа добавлены `legacy.bitrixElementId` и `paidAt`.
- Добавлен идемпотентный импорт `import:legacy-payments`, связывающий платёж с пользователем по `legacy.bitrixId`.
- Импортировано 1 156 платежей со статусом `paid`; суммы, transaction ID, время оплаты и исходные реквизиты сохранены.
- Для четырёх платежей с отсутствующим в пользовательской выгрузке Bitrix ID `6` создана одна служебная запись «Удалённый пользователь» без доступа.
- История платежей не меняет сроки подписки пользователей.

Изменённые файлы:

- `apps/api/src/models/Payment.ts`
- `apps/api/src/scripts/importLegacyBitrixPayments.ts`
- `apps/api/package.json`
- `docs/migration-log.md`
...
- `npm run build --workspace @socstat/api`
- `npm run test --workspace @socstat/api`
- Агрегированная сверка MongoDB: 1 156 исторических платежей, все привязаны к пользователям, нет дубликатов исторических ID и transaction ID.

## 2026-08-25 - Сводка последних платежей в админке

Запрос пользователя:

- Добавить информацию о последних 100 платежах в админку.

Результат:

- Раздел явно обозначен как «Последние 100 платежей»; серверный лимит уже применялся к выборке.
- Добавлена сводка по показанным платежам: количество, число оплаченных и общая сумма.
...
- `npm run build --workspace @socstat/frontend`

## 2026-08-25 - Единый формат дат в админке

Запрос пользователя:

- Выводить даты в админке в формате `dd.mm.yyyy`.

Результат:

- Даты платежей и сроков доступа отображаются в формате `dd.mm.yyyy`.
- Для последнего входа сохранено время в формате `dd.mm.yyyy, HH:mm`.
...
## 2026-08-25 - Внутренняя прокрутка списка последних входов

Запрос пользователя:

- Ограничить высоту блока последних активных пользователей и добавить внутренний скролл.

Результат:

- Высота списка ограничена `620px`; переполнение прокручивается внутри блока.
- Заголовок таблицы закреплён при прокрутке.
...
## 2026-08-25 - Внутренняя прокрутка таблицы платежей

Запрос пользователя:

- Ограничить высоту таблицы платежей.

Результат:

- Высота таблицы платежей ограничена `620px`; добавлена внутренняя прокрутка и закреплённый заголовок.
...
## 2026-08-25 - Сводка платежей за два месяца в админке

Запрос пользователя:

- Добавить в админку информацию о платежах за текущий и прошлый месяцы.

Результат:

- Добавлен защищённый маршрут `GET /api/payments/admin/summary`.
- В админке отображаются сумма и число оплаченных платежей за текущий и предыдущий календарные месяцы.
- Для исторических платежей используется `paidAt`, для новых — `createdAt`.
