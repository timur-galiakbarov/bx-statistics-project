...
angular
    .module('rad.stat')
    .controller('postsDashboardController', ['$rootScope', '$scope', 'bus', 'appState', 'vkApiFactory', '$timeout', 'radCommonFunc', 'notify', '$state',
        function ($rootScope, $scope, bus, appState, vkApiFactory, $timeout, radCommonFunc, notify, $state) {
            ...
            $rootScope.setTitle("Анализ публикаций");
            ...
            function addToCompareGroup(event, group) {
                ...
                if (group) {
                    ...
                    if (!appState.isActiveUser() && !appState.groupIsFree(group)){
                        notify.error("Эту группу нельзя добавлять в список сравнения, так как закончился период подписки безлимитного доступа.");
                        return;
                    }
                    ...
                    if (!foundGroup.length) {
                        ...
                        notify.success("Сообщество добавлено в список для анализа");
                    } else {
                        notify.error("Сообщество уже находится в списке для анализа постов");
                    }
                } else {
