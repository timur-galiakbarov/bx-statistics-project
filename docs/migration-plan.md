...
- `apps/frontend` - приложение на React + Vite.
- `apps/api` - API на Node.js + Express.
...
Заменяем текущее временное хранилище в `apps/api` на модели Mongoose.

Критерии готовности:

- В `apps/api/.env` поддерживается `MONGO_URI`.
