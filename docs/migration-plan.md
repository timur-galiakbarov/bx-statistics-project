...
- `users`
  - VK id, имя, аватар, роль, дата окончания оплаченного периода доступа.
- `sessions`
...
- Дата окончания оплаченного периода доступа продлевается после подтверждённой оплаты.
...
Основной маршрут переключаем только после того, как авторизация, статус доступа, сохранённые группы, dashboard и ключевая аналитика работают через MongoDB и Node.js.
